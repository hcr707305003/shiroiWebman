...
class AdminMenus extends AbstractSeed
{
    /**
     * Run Method.
     *
    ...
    public function run(): void
    {
        ...
        //后台角色列表
        $AdminRole = $this->getData('admin_role', ['id' => 1]);

        //初始化hash值
        $AdminMenuList = $this->getData('admin_menu');
        foreach ($AdminMenuList as $v){
            if (empty($v['hash'])){
                $this->updateData(['id' => $v['id']], ['hash' => md5($v['name'].$v['url'])]);
            }
        }
        ...
        foreach ($AdminRole as $v) {
            $AdminRoleUrls[$v['id']] = array_column($this->getData('admin_menu', ['id' => ['in', "({$v['url']})"]]), 'hash');
        }
        //截断表
        $this->query('truncate table admin_menu');
        self::menuSeed($data, 0);
        //新菜单数据
        foreach ($AdminRoleUrls as $k => $v) {
            if ($k == 1) { //角色id=1, 填充所有菜单url权限
                $NewAdminMenu = $this->getData('admin_menu');
                $url_str = implode(',', array_column($NewAdminMenu, 'id'));
                $this->updateData(['id' => $k], ['url' => $url_str], 'admin_role');
            } else {
                $NewAdminMenu = $this->getData('admin_menu');
                $url_str = implode(',', array_column($NewAdminMenu, 'id'));
                $this->updateData(['id' => $k], ['url' => $url_str], 'admin_role');
    ...
    public function menuSeed($menu = [], $id = 0)
    {
        ...
        try {
            foreach ($menu as $k => $v) {
                ...
                $menu_id = $this->insertData($v, 'admin_menu');
                if (!empty($Sons)) {
    ...
    public function getData($table = 'admin_menu', $where = []): array
    {
        $whereArr = [];
        ...
        foreach ($arr = $this->fetchAll("SELECT * FROM {$table}" . ($whereArr? ' where ' . implode(' and ', $whereArr) : '')) as $k => $v) {
            $arr[$k] = array_filter($v, function($key) {
    ...
    public function insertData($data, $table = 'admin_menu')
    {
        $this->insert($table, $data);
        return $this->fetchRow("select * from `{$table}` where hash = '{$data['hash']}'")['id'];
    }

    public function updateData($where = [], $updateData = [], $table = 'admin_menu')
    {
        $updateArr = [];
        $whereArr = [];
        ...
        $this->query("update `{$table}` set " . (implode(', ', $updateArr)) . ' where ' . (implode(' and ', $whereArr)));
    }
}
