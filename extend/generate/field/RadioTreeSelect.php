...
<div class="form-group row rowMultiSelect [FIELD_NAME]-form-group">
...
EOF;
    ...
    public static function create($type, $name, $field, $content, $option, $type_arr): string
    {
        ...
        if(is_array($type_arr)) {
            $collapsed = $type_arr['collapsed']??true;
        }
...
//        dd($type, $name, $field, $content, $option);
        return str_replace(array('[FORM_NAME]', '[FIELD_NAME]', '[OPTION_DATA]', '[ID_STR]', '[DEFAULT_VALUE]','[STARTCOLLAPSED]'), array($name, $field, $str, $arr?implode(',',$arr):'', $content, $collapsed), $html);
    }
}